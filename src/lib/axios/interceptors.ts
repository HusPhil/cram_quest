...
// Always attach the current access token and API key to every request
axiosInstance.interceptors.request.use((config) => {
  const token = useAuthInformationStore.getState().accessToken;
  console.log(token);
  if (token) {
...
axiosInstance.interceptors.response.use(
  ...
  async (error) => {
    ...
    try {
      ...
      useAuthInformationStore.getState().setAcessToken(newToken);

      ...
      {bossBattle && battleSessionResult ? (
        <div className="mt-5 w-full flex flex-col items-center">
          ...
          <div className="flex w-full gap-5">
            ...
            {rewardItem && (
              <BattleResultButton
                title={isViewingReward ? "Results" : "Reward"}
                className={!isViewingReward ? "animate-pulse" : ""}
                result={result}
                onClick={() => setIsViewingReward((prev) => !prev)}
              />
            )}
          </div>
        </div>
        ...
        <>
          <BattleResultStats
            ...
            bonus_xp={battleSessionResult.bonus_xp}
            streak_count={
              (battleSessionResult as BattleSessionEnd).session_streak
            }
          />
          {(battleSessionResult as BattleSessionEnd).is_boss_available && (

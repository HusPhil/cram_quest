...
const NavItem = ({ path, label, icon, isActive, isCollapsed, isMobileOpen }: NavItemProps) => {
    return (
        <Link
        to={path}
        className={`
            ...
            ${isCollapsed ? 'md:justify-center px-2' : 'px-3'} py-2.5
        ...
        <span className={`
            ...
            ${isCollapsed ? 'md:hidden' : 'opacity-100 ml-3'}
            ${isMobileOpen && 'ml-3'}
            ${isActive ? 'text-amber-400' : 'text-gray-400 group-hover:text-amber-400'}
        `}>
            {label}
